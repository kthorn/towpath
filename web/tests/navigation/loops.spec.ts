...
function journey(totalKm: number, days: number) {
  return {
    route: {
      ...
      is_ring: true,
      legs: [{
        from_place: 'Origin base', to_place: 'Circuit junction', distance_km: totalKm,
        locks: 2, est_minutes: 120, flagged_unknown_dims: false,
      }],
      days: Array.from({ length: days }, (_, index) => ({
        day: index + 1, legs: [], end_near: index + 1 === days ? 'Origin base' : 'Circuit junction', cruising_minutes: 120,
      })),
      total_km: totalKm,
      total_locks: 2,
...
const routes = [
  {
    ...
    selection_basis: 'longest_feasible' as const,
    budget: { available_minutes: 2520, used_minutes: 120, remaining_minutes: 2400, days_used: 1 },
  ...
  },
  {
    ...
    loop_distance_km: 14,
    connecting_distance_km: 0,
    selection_basis: 'longest_feasible' as const,
    budget: { available_minutes: 2520, used_minutes: 240, remaining_minutes: 2280, days_used: 2 },
...
async function installFixtures(page: Page) {
  ...
  await page.route('**/api/loop-candidates', (route) => {
...
test('origin-only loop submits a null waypoint', async ({ page }) => {
  const requests = await installFixtures(page);
  await openLoop(page);
  await page.getByRole('button', { name: 'Plan loop journey' }).click();
  await expect(page.getByRole('region', { name: 'Loop routes' })).toBeVisible();
...
test('shows both circuit shapes and switches the exact journey and day geometry', async ({ page }) => {
  const requests = await installFixtures(page);
  await openLoop(page);
  ...
  const options = page.getByRole('region', { name: 'Loop routes' }).getByRole('button');
  await expect(options).toHaveCount(2);
  await expect(options.nth(0)).toContainText('Circuit with return connection');
  await expect(options.nth(1)).toContainText('Circuit through base');
...
test('constraint edits invalidate the displayed loop route and branch selection', async ({ page }) => {
  await installFixtures(page);
  await openLoop(page);
  await page.getByRole('button', { name: 'Plan loop journey' }).click();
  await expect(page.getByRole('region', { name: 'Loop routes' })).toBeVisible();
  await page.getByLabel('Days').fill('8');
  await expect(page.getByRole('region', { name: 'Loop routes' })).toHaveCount(0);
