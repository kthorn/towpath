...
def test_inventory_reports_user_facing_kinds_and_exclusions():
    report = inventory_pbf(Path("tests/fixtures/tiny_bulk.osm"))
...
def test_inventory_deduplicates_duplicate_source_object(tmp_path):
    ...
    source.write_text(
        Path("tests/fixtures/tiny_bulk.osm")
        .read_text()
        .replace(
            "</osm>",
...
def test_inventory_rejects_missing_source():
    ...
    with pytest.raises(FileNotFoundError):
        inventory_pbf(Path("tests/fixtures/missing.osm.pbf"))
...
def test_inventory_cli_writes_sorted_json_and_rejects_repo_data_output(tmp_path):
    ...
    assert main(["--pbf", "tests/fixtures/tiny_bulk.osm", "--out", str(output)]) == 0
    ...
    with pytest.raises(ValueError, match="repository data or artifact directory"):
        main(["--pbf", "tests/fixtures/tiny_bulk.osm", "--out", "pound/data/inventory.json"])

    missing_output = tmp_path / "missing.json"
    with pytest.raises(FileNotFoundError):
        main(["--pbf", "tests/fixtures/missing.osm.pbf", "--out", str(missing_output)])
