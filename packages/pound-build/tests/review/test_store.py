...
import pytest
from pound_build.review.store import ReviewFileError, ReviewStore, load_document, write_document
...
def test_failed_atomic_replace_preserves_previous_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("pound_build.review.store.os.replace", fail_replace)
...
def test_store_serializes_concurrent_decisions(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("pound_build.review.store.write_document", pause_first_write)
    failures = []
