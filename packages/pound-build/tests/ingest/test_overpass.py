...
from fixtures import oxford_fixture_path, poi_fixture_path
from pound.models import WaterwayKind  # pyright: ignore[reportMissingImports]
from pound_build.ingest.ir import NodeKind
from pound_build.ingest.overpass import OXFORD_BBOX, build_query, fetch_oxford, parse
from shapely import wkt

FIXTURE = oxford_fixture_path()
POI_FIXTURE = poi_fixture_path()
...
def test_fetch_oxford_applies_prune_then_filter_chain(monkeypatch):
    ...
    from pound_build.ingest import overpass as _overpass
...
def test_parse_emits_deduplicated_ordered_poi_candidates_with_normalized_tags():
    ...
    relation_geometry = wkt.loads(relation.geometry_wkt)
    assert relation_geometry.geom_type == "Polygon"
    assert len(relation_geometry.interiors) == 1  # pyright: ignore[reportAttributeAccessIssue]
