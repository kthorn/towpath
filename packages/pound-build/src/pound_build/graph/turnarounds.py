...
from pound.turnarounds import (
    _LIMIT_FIELDS,
    _RECORD_REQUIRED,
    _SOURCE_REQUIRED,
    validate_turnarounds,
)
    ...
    *,
    ...
    for node in mapped_nodes:
        ...
        if not any(
            ...
            report["unmatched"].append(f"node/{node.osm_id}")
            continue
        result.nodes[uid]["turning_point"] = True
        maximum = _positive_limit(node.tags, ("maxlength", "max_length"))
        if maximum is not None:
            previous_maximum = result.nodes[uid].get("turning_max_length_m")
            result.nodes[uid]["turning_max_length_m"] = (
                maximum
                if previous_maximum is None
                else min(float(previous_maximum), maximum)
            )
