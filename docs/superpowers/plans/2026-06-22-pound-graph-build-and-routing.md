...
- Produces: `plan_route(constraints: CanalConstraints) -> RouteResult`. Honors `constraints.days` and `constraints.hours_per_day` via `_chunk_days(legs, hours_per_day, max_days) -> list[DayPlan]` (greedy cumulative-minute packing; non-empty days only, no padding to `days` — OQ-8). **Production artifact loading is deferred to Scope D** (Agent Core integration): in Scope C, calling `plan_route` without the test-only kwargs raises `RuntimeError("artifact loading not wired in this scope")`. The Scope C goal — a real computed `RouteResult` over the curated fixture — is met via test injection: `plan_route` accepts optional `_graph: nx.Graph | None = None, _features: WaterwayFeatures | None = None` kwargs that build the graph directly from features (no pickle IO) so tests stay fast and hermetic. When Scope D wires production loading, it will add a module-level cache that `load_artifact`s from a default path (or `POUND_ARTIFACT_PATH` env var) on first call; that contract (cache policy, path resolution, error behavior) is designed against a real caller then, not blind now. (The earlier draft of this prose promised the cache in Scope C — corrected to match the code after the final whole-branch review.)
