...
describe('Google SDK production bridge', () => {
  it('wires Maps, Places, and AdvancedMarkerElement constructors', () => {
    ...
    const adapters = createGoogleAdapters(
      { maps: { Map: MapCtor, Polyline: PolylineCtor, InfoWindow: InfoWindowCtor }, marker: { AdvancedMarkerElement: MarkerCtor }, places: { PlaceAutocompleteElement: PlaceAutocompleteElementCtor }, routes: { Route: { computeRoutes: vi.fn() }, RouteMatrix: { computeRouteMatrix: vi.fn() } } },
      { mapId: 'pound-map' },
    );
