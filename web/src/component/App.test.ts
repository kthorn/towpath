...
const endpoint = (name: string, uid: number, unavailable = false) => ({
	...
	candidates: [
		{
			...
			recommended: true,
			...(unavailable
				? { available: false as const, reason: "ZERO_RESULTS" }
				: {
	...
	landRoute: unavailable
		? null
		: { path: [], durationSeconds: 600, distanceMeters: 2400 },
	transferWarning: unavailable ? "Could not verify a land transfer." : null,
	requiresManualConfirmation: unavailable,
...
const route = {
	route: {
		...
		is_ring: false,
		legs: [],
		days: [{ day: 1, legs: [], end_near: "Braunston", cruising_minutes: 360 }],
		total_km: 21.4,
		total_locks: 7,
		total_minutes: 420,
		amenities: [],
	...
	geometry: {
		...
		coordinates: [[-1, 51] as [number, number]],
	},
};
...
function setup(
	...
	const calls: Array<{
		slot: EndpointSlot;
		place: SelectedPlace | { lat: number; lon: number };
	}> = [];
	...
	const dependencies: AppDependencies = {
		store,
		placeSearch: {
			attach: vi.fn((_input, onSelect) => {
				selects.push(onSelect);
				return vi.fn();
			}),
		},
		loadMapView: overrides.mapReject
			? vi.fn(async () => {
...
describe("trip planning interface", () => {
	...
	it("returns to the planner and reports a persistent save there", async () => {
		...
		expect(
			screen.getByRole("heading", { name: "Plan your canal journey" }),
		).toBeVisible();
		await waitFor(() =>
	...
	it("restores saved dimensions and includes them in route planning", async () => {
		...
		expect(store.planCanalRoute).toHaveBeenCalledWith(
	...
	it("rejects invalid boat settings without replacing saved values", async () => {
		...
		await fireEvent.input(screen.getByLabelText(/boat beam/i), {
			target: { value: "-1" },
		});
		await fireEvent.click(
	...
	it("does not offer manual latitude or longitude entry", () => {
		...
		expect(
			screen.queryByRole("button", { name: /use coordinates/i }),
	...
	it("connects both place searches and map clicks to the active endpoint", async () => {
		...
		await fireEvent.click(
			screen.getByRole("radio", { name: /set destination from map/i }),
		);
		...
		expect(calls).toEqual([
			...
			{
				slot: "destination",
				place: expect.objectContaining({ name: "Marina" }),
			},
	...
	it("shows candidate recommendation, metrics, unavailable reasons, and confirmation", async () => {
		...
		expect(
			within(origin).getByText(/unavailable: zero results/i),
		).toBeVisible();
		expect(within(origin).getByText(/recommended/i)).toBeVisible();
		expect(within(origin).getByRole("alert")).toHaveTextContent(
			/could not verify/i,
		);
		await fireEvent.click(
			within(origin).getByRole("button", { name: /confirm geometric/i }),
		);
	...
	it("defaults and resets the required schedule controls", async () => {
		...
		expect(newHosts).toHaveLength(2);
		expect(newHosts[0]).not.toBe(oldHosts[0]);
		expect(newHosts[1]).not.toBe(oldHosts[1]);
		expect(oldHosts.every((host) => !document.body.contains(host))).toBe(true);
	...
	it.each(["", "0", "-2"])("blocks invalid hours per day %j", async (value) => {
		...
		expect(screen.getByRole("alert")).toHaveTextContent(
			/hours per day.*greater than 0/i,
		);
	});
	...
	it("renders route metrics, transfers, warnings and days", () => {
		...
		expect(
			screen.getByRole("button", { name: /day 1.*6 hr.*end near Braunston/i }),
		).toBeVisible();
	...
	it("toggles catalog layers and selects a route day without replanning", async () => {
		...
		render(App, {
			props: { dependencies: { ...fixture.dependencies, store } },
		});

		await fireEvent.click(
			screen.getByRole("checkbox", { name: "Hospitality" }),
		);
	...
	it("uses the exact same-node label without empty day chrome", () => {
		render(App, {
			props: { dependencies: setup({ sameNode: true }).dependencies },
		});
	...
	it("does not use the same-node label for a rounded-zero route with legs", () => {
		...
		expect(
			screen.queryByText("No canal travel required"),
		).not.toBeInTheDocument();
	});
	...
	it("uses real route links and preserves App-owned schedule values", async () => {
		...
		await fireEvent.click(settings);
		expect(
			screen.getByRole("heading", { name: /boat settings/i }),
		).toBeVisible();
		expect(screen.getAllByRole("main")).toHaveLength(1);
	...
	it("preserves planner state across settings visit", async () => {
		...
		expect(store.setEndpointCoordinate).toHaveBeenCalledWith(
			"origin",
	...
	it("tears down and recreates the map across settings round trip", async () => {
		...
		await fireEvent.click(screen.getByRole("link", { name: /plan trip/i }));
		await vi.waitFor(() =>
			expect(store.setMapView).toHaveBeenCalledWith(secondMap),
		);
	...
	it("cleans up both searches and the map on unmount", async () => {
		const fixture = setup();
		const detach = [vi.fn(), vi.fn()];
		let index = 0;
		fixture.dependencies.placeSearch.attach = vi.fn(
		...
		const rendered = render(App, {
			props: { dependencies: fixture.dependencies },
		});
	...
	it("ignores a map rejection that arrives after unmount", async () => {
		...
		fixture.dependencies.loadMapView = vi.fn(
			() =>
				new Promise<MapView>((_resolve, rejectPromise) => {
	...
	it("updates title and focuses the destination heading after navigation", async () => {
		...
		expect(
			screen.getByRole("heading", { name: "Boat settings" }),
		).toHaveFocus();
	...
	it("settings page at narrow viewport has no planner controls in active DOM", async () => {
		...
		await fireEvent.click(screen.getByRole("link", { name: "Settings" }));
		expect(
			screen.queryByRole("button", { name: /plan canal route/i }),
		).not.toBeInTheDocument();
...
it('paints the grid surface, adjusts opacity without fetching, and clears on disable', async () => {
  ...
  await waitFor(() => expect(map.climateGrid).toHaveBeenCalledWith(surface, 0.4, null));
  gridStore.setOpacity(0.6);
  await waitFor(() => expect(map.climateGrid).toHaveBeenLastCalledWith(surface, 0.6, null));
  await gridStore.setColorRange({ min: 18, max: 30 });
  await waitFor(() => expect(map.climateGrid).toHaveBeenLastCalledWith(surface, 0.6, { min: 18, max: 30 }));
  await gridStore.setColorRange(null);
  await waitFor(() => expect(map.climateGrid).toHaveBeenLastCalledWith(surface, 0.6, null));
  ...
  await waitFor(() => expect(map.climateGrid).toHaveBeenLastCalledWith(null, 0.6, null));
});
