...
from pound.route._round_trip_render import (
    _render_path,
    _route_response,
    _segment_costs,
    _traversal_time_min,
...
POLICY_VERSION = "out-and-back-branches-v2"
MAX_WORK = 100_000
MAX_ROUTES = 1_000
...
def _routing_eligible(data: dict) -> bool:
    tags = {
        **(data.get("tags") or {}),
        **{key: data[key] for key in ("boat", "access") if key in data},
    }
    return (
        data.get("routing_eligible", True) is not False
        and data.get("navigable", True) is not False
        and tags.get("boat") not in {"no", "unsuitable", "canoe", "private", "permit"}
        and tags.get("access") not in {"no", "private", "permit"}
    )
...
def _request_values(body: TurnaroundCandidatesRequest) -> dict:
    values = body.model_dump(include=set(TurnaroundCandidatesRequest.model_fields))
...
def _projected_graph(body: TurnaroundCandidatesRequest, graph: nx.Graph):
    """Insert request-local vertices for exact compact-edge positions."""
    handles = {
        field: getattr(body, field)
        for field in ("start", "waypoint")
        if getattr(body, field) is not None
    }
    if not handles:
        return body, graph
    from pound.route.plan import _edge_geometry, _edge_record, _validate_handle
    from pound.route.project import project_handle

    for field, handle in handles.items():
        try:
            _validate_handle(handle, graph, field)
        except ValueError as exc:
            raise RoundTripError(
                "invalid_node_handle", str(exc), fields=[field], status=400
            ) from exc
    updates = {}
    interior = {}
    next_uid = max(graph.nodes, default=-1) + 1
    for field, handle in handles.items():
        if handle.fraction in (0, 1):
            updates[field + "_uid"] = handle.edge[int(handle.fraction)]
        else:
            key = (handle.edge, handle.fraction)
            if key not in interior:
                interior[key] = next_uid
                next_uid += 1
            updates[field + "_uid"] = interior[key]
    if not interior:
        return body.model_copy(update=updates), graph
    local = graph.copy()
    for (edge, fraction), uid in interior.items():
        point = project_handle(CanalPointHandle(edge=edge, fraction=fraction), graph).coordinate
        local.add_node(uid, lat=point.lat, lon=point.lon, movable_bridge_ids=())
    for edge in sorted({edge for edge, _ in interior}):
        data = graph.edges[edge]
        points = [(0.0, edge[0]), (1.0, edge[1])]
        points.extend(
            (fraction, uid) for (candidate, fraction), uid in interior.items() if candidate == edge
        )
        points.sort()
        local.remove_edge(*edge)
        for (low, u), (high, v) in zip(points, points[1:], strict=False):
            local.add_edge(
                u,
                v,
                **{
                    **data,
                    "geometry": _edge_geometry(_edge_record(edge, low, high), graph),
                    "length_m": data["length_m"] * (high - low),
                    "locks": 0,
                    "lock_points": (),
                    "movable_bridge_ids": (),
                },
            )
    return body.model_copy(update=updates), local
...
def discover_round_trips(
    ...
    *,
    ...
    constraints = CanalConstraints(**_request_values(body))
    segment_cache = {}

    def segment_costs(u, v):
    ...
    forward_costs: list[list[float]] = []
    reverse_costs: list[list[float]] = []
    ...
    while stack:
        ...
        path.append(uid)
        visited.add(uid)
        forward_costs.append(segment_costs(previous, uid))
        reverse_costs.append(segment_costs(uid, previous))
        ...
        if turn is not None and seen:
            ...
            ranges, used, legs_fit = _schedule(
                [cost for segment in forward_costs + reverse_costs[::-1] for cost in segment],
                body.hours_per_day,
            )
    ...
    for choice in ordered:
        ...
        vertices += 2 * count + 2 * sum(
            len(segment_costs(u, v)) for u, v in zip(closed, closed[1:], strict=False)
        )  # Whole/day geometry includes the compact reporting segment boundaries.
