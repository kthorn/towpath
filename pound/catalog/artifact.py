"""Strict serialization for the independent OSM place catalog."""
...
_ALLOWED_GLOBALS = {
    ...
    ("pound.ingest.ir", "OsmElementType"),
}
...
class InvalidCatalogError(ValueError):
    """A catalog pickle or record does not satisfy the strict catalog contract."""
...
class CatalogArtifact:
    ...
    def __post_init__(self) -> None:
        places = _parse_places(self.places)
...
def write_catalog(artifact: CatalogArtifact, path: Path) -> None:
    """Write exactly one independently validated catalog payload."""
    if not isinstance(artifact, CatalogArtifact):
        raise _invalid("artifact", artifact, "expected CatalogArtifact")
    artifact = CatalogArtifact(places=artifact.places, metadata=artifact.metadata)
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("wb") as stream:
        pickle.dump({"places": list(artifact.places), "metadata": artifact.metadata}, stream)


def load_catalog(path: Path) -> CatalogArtifact:
    """Load and strictly validate a trusted local catalog pickle."""
    try:
        with Path(path).open("rb") as stream:
            payload = _CatalogUnpickler(stream).load()
        ...
        return CatalogArtifact(places=payload["places"], metadata=payload["metadata"])
    except InvalidCatalogError:
        raise
    except Exception as exc:
        raise _invalid("pickle", str(path), f"could not load catalog: {exc}") from exc
