...
Detailed setup, API, artifact, and data guidance for Towpath and the Pound routing engine.
Run commands from the repository root unless a section says otherwise.
See the [project README](../README.md) for the quick start.
...
curl -X POST http://127.0.0.1:8000/api/turnaround-candidates \
  ...
  -d '{"artifact_revision":"YOUR_REVISION","start":{"edge":[123,456],"fraction":0.25},"days":3,"hours_per_day":6}'
```
...
A supplied `waypoint` edge/fraction handle must be visited before turning. All costs include the return trip;
...
[deployment runbook](fly-runbook.md#optional-historical-temperature-artifacts) for
