...
def test_return_counts_locks_and_directional_bridges_and_days():
    ...
    # Lock-event segmentation conservatively packs raw/rounded reporting costs.
    assert result.budget.used_minutes == pytest.approx(39)
    assert sum(d.cruising_minutes for d in result.journey.route.days) == 38
    assert LineString(result.journey.day_geometries[0].geometry.coordinates).hausdorff_distance(
        LineString(result.journey.geometry.coordinates)
    ) < 1e-6
