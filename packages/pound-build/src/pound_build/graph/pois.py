...
from pound_build.ingest.filters import is_navigable
from pound_build.ingest.ir import PoiCandidate, PointOfInterest
...
def _xy_to_pound(point: Any) -> tuple[float, float]:
...
def _to_wgs84(geometry):
    return transform(geometry, cast(Any, _TO_WGS84.transform), interleaved=False)
    ...
    def attach_many(
        ...
        if accepted:
            ...
            for accepted_index, (
                ...
                geometry_wgs84 = cast(Any, geometries[candidate_index])
                geometry_bng = cast(Any, geometries_bng[local_index])
